	...
	if pixel_is_player:	
		...
		match current_state:
			States.IDLE: # stanje po vsake koraku oz potezi
				...
				# skill ready
				if detect_collision_in_direction(vision_ray, direction):# and detect_collision_in_direction(vision_ray, direction).is_in_group(Global.group_strays): # more bit neodvisno od stateta, da pull dela
...
#	Global.main_camera.global_position += Vector2(100,100) 
#	printt ("camera", Global.main_camera.global_position)
