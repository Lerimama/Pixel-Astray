...
var lines_scrolled_count: int = 0
var lines_scroll_limit: int = 7 # kot je visoka površina za spawnanje
var lines_scroll_pause: int # game_settings je definiram že v defolt GM
#func _ready() -> void:
#
...
func set_strays():
	...
	# SCOLLER ... 1 ekran tudi v prmeru dveh plejerjev
	if lines_scrolled_count < 14:
		lines_scroll_pause = game_settings["scrolling_pause_time"]*5
		
		spawn_strays(game_data["strays_start_count"]/10)
	elif lines_scrolled_count < 28:
		lines_scroll_pause = game_settings["scrolling_pause_time"]*2
		spawn_strays(game_data["strays_start_count"]/5)
	elif lines_scrolled_count < 36:
		spawn_strays(game_data["strays_start_count"]/2)
		lines_scroll_pause = game_settings["scrolling_pause_time"]
	else: 
	...
	print ("lines_scrolled_count ", lines_scrolled_count)
	print ("lines_scroll_pause ", lines_scroll_pause)
...
func stray_step():
	...
	# SCROLLER
#	if game_data["game"] == Profiles.Games.SCROLLER:
	var stepping_direction: Vector2 = Vector2.DOWN
	
	for stray in get_tree().get_nodes_in_group(Global.group_strays):
		if stray.current_state == stray.States.IDLE: 
			stray.step(stepping_direction)
	
	yield(get_tree().create_timer(lines_scroll_pause), "timeout")
	
	if lines_scrolled_count % lines_scroll_limit == 0:
