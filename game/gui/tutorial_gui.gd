...
func finish_travel(): 
	...
	if not current_tutorial_stage == TutorialStage.TRAVEL:
		return	
#	change_stage(travel_content, multicollect_content, TutorialStage.MULTICOLLECT) # debug
...
func finish_tutorial():
	...
	current_tutorial_stage = TutorialStage.IDLE
	tutorial_on = false
	
	var hint_fade = get_tree().create_tween()
	hint_fade.tween_property(finish_hint, "modulate:a",0, 0.5).from(0).set_ease(Tween.EASE_IN)
	hint_fade.tween_callback(finish_hint, "hide")
	animation_player.play("tutorial_end_with_sidebar")
	
	# če se je končal na GO
...
func on_color_collected(colors_collected_count: int):
...
func _on_AnimationPlayer_animation_finished(anim_name: String) -> void:
	...
	match anim_name:
		"tutorial_start_with_sidebar":
			...
			finish_hint.show()
			var hint_fade = get_tree().create_tween()
			hint_fade.tween_property(finish_hint, "modulate:a",1, 0.7).from(0).set_ease(Tween.EASE_IN)
