...
onready var halls: Array = [
	...
	$ScrollContent/Unbeatables/TabContainer/StalkerHall, 
...
#		var hall_table_global: Control = hall.get_node("TablePair/HighscoreTableGlobal")
		var hall_table_global: Control = hall.get_node("HighscoreTableGlobal")
...
#		var hall_table_global: Control = sweeper_hall.get_node("TablePair/HighscoreTableGlobal")
		var hall_table_global: Control = sweeper_hall.get_node("HighscoreTableGlobal")
		# novo hall ime, ker se vidi v tabih
...
func load_all_highscore_tables(update_tables: bool = false):
	...
	for table in all_local_tables:
		...
		# najprej pridobim podatke in zapišem tabele
		
		
#		table.load_highscore_table(game_data_local, fake_player_ranking, 50)
		table.load_highscore_table(game_data_local, fake_player_ranking, Profiles.local_highscores_count)
		# potem mergam z lokal rankom
	...
	for table in all_global_tables:
		...
		# hide titles
		var show_table_title: bool = false
#		if game_data_local["game"] == Profiles.Games.CLEANER:
#			show_table_title = true
			
		table.load_highscore_table(game_data_local, fake_player_ranking, Profiles.global_highscores_count, true, show_table_title)
